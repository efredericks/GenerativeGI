...
def evalOneMax(individual):
    rng = random.Random(0)
    image = Image.new("RGBA", DIM, "#000000")
    strange_attractor(image, rng, random.choice(palettes), DIM, individual)
...
def mutate(individual):
    ...
    mutant[idx] = random.uniform(-100.0, 100.0)
    return mutant
...
toolbox.register("mutate", mutate)#tools.mutFlipBit, indpb=0.05)
...
def main():
    ...
    while g < 1000:
        ...
        #     current_fitnesses.append(ind.fitness.values[0])
        current_stats = stats.compile(fits)
        current_stats['Generation'] = g
        logbook.append(current_stats)
    ...
    plt.plot(logbook.select('Generation'), logbook.select('max'))

    plt.title("Max values",fontsize=20,fontweight='bold')
    plt.xlabel("Generations",fontsize=18,fontweight='bold')
    plt.ylabel("Non-black pixels",fontsize=18,fontweight='bold')
