...
def singlePointMutation(ind):
    ...
    if ind.rng.random() < 0.25:
        ...
        # class, they really should but... for now this will do
        if technique == "rectangle":
            # x, y, width, height, fill
            gene[1] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x
            gene[2] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y
            gene[3] = str(ind.rng.randint(1, DIM[0] - int(gene[1])))  # Mutate width
            gene[4] = str(ind.rng.randint(1, DIM[1] - int(gene[2])))  # Mutate height
            gene[5] = str(ind.rng.randint(0, 255))  # Mutate fill
        elif technique == "circle":
            # x, y, radius, fill
            gene[1] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x
            gene[2] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y
            gene[3] = str(ind.rng.randint(1, min(DIM[0], DIM[1])//4))  # Mutate radius
            gene[4] = str(ind.rng.randint(0, 255))  # Mutate fill
        elif technique == "triangle":
            # x1, y1, x2, y2, x3, y3, fill
            gene[1] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x1
            gene[2] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y1
            gene[3] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x2
            gene[4] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y2
            gene[5] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x3
            gene[6] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y3
            gene[7] = str(ind.rng.randint(0, 255))  # Mutate fill
        elif technique == "hexagon":
            # x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, fill
            gene[1] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x1
            gene[2] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y1
            gene[3] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x2  
            gene[4] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y2
            gene[5] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x3
            gene[6] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y3
            gene[7] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x4
            gene[8] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y4
            gene[9] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x5
            gene[10] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y5
            gene[11] = str(ind.rng.randint(0, DIM[0]-1))  # Mutate x6
            gene[12] = str(ind.rng.randint(0, DIM[1]-1))  # Mutate y6
            gene[13] = str(ind.rng.randint(0, 255))  # Mutate fill
